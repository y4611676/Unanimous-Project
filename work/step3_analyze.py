...
def rfm_analysis(cust_agg):
    ...
    def segment(row):
        ...
        if r >= 3 and f >= 3:            return "忠實"
        if r <= 2 and f >= 3:            return "沉睡"
        if r >= 3 and m >= 3:            return "高消費"
        if r <= 2 and f <= 2:            return "流失邊緣"
        return "一般"
...
def pareto_analysis(df, value_col, name_col):
    ...
    df["pareto"] = df["cumulative_share"].apply(lambda x: "前 80%" if x <= 0.8 else "後 20%")
    return df
...
def sheet_summary(wb, monthly, cust_agg, prod_agg, stock_agg):
    ...
    ws = wb.create_sheet("業務總覽")
    page_title(ws, "業務總覽", 8)
    ...
    kpis = [
        ("銷售總額",   total_sale, "#,##0",  MED_BLUE),
        ("進貨總額",   total_purc, "#,##0",  DARK_BLUE),
        ("毛利總額",   total_gp,   "#,##0",  GREEN if total_gp >= 0 else RED),
        ("平均毛利率", gp_rate,    "0.0%",   GREEN_SOFT),
        ("活躍客戶數", cust_cnt,   "#,##0",  ORANGE),
        ("商品數",     prod_cnt,   "#,##0",  "7030A0"),
    ]
    ...
    # Monthly trend table
    row = 11
    title(ws, row, 1, "月度趨勢", 7)
    row += 1
    for ci, h in enumerate(["月份","銷售額","進貨額","毛利","毛利率","銷進差","累計銷售"], 1):
...
def sheet_rfm(wb, rfm_df):
    ...
    ws = wb.create_sheet("客戶 RFM 分析")
    page_title(ws, "客戶 RFM 分群", 9)
    if rfm_df.empty:
        ws["A2"] = "資料不足"; return
    ...
    title(ws, 2, 1, "分群統計", 5)
    for ci, h in enumerate(["分群","客戶數","銷售總額","平均銷售","占比"], 1):
        hdr(ws, 3, ci, h)
    ...
    hdrs_map  = {"cusno":"客戶編號","cusnm":"客戶名稱","segment":"分群",
                 "order_count":"訂單數","sales":"銷售額","avg_order_value":"平均單價",
                 "R_days":"距今天數","RFM_score":"RFM 分數"}
...
def sheet_pareto(wb, prod_agg, cust_agg):
    ...
    ws = wb.create_sheet("80-20 分析")
    page_title(ws, "80/20 分析", 8)

    # Product Pareto
    title(ws, 2, 1, "商品銷售 80/20（前 80% 貢獻者）", 8)
    if not prod_agg.empty:
        prd = pareto_analysis(prod_agg.copy(), "sales", "prdno")
        hdrs_p = ["排名","商品編號","商品名稱","銷售額","毛利率","占比","累計占比","80/20"]
        for ci, h in enumerate(hdrs_p, 1):
            hdr(ws, 3, ci, h)
        for i, r in prd.reset_index(drop=True).iterrows():
            row = i + 4
            is_top = r["pareto"] == "前 80%"
    ...
    # Customer Pareto
    title(ws, prd_end, 1, "客戶銷售 80/20", 8)
    if not cust_agg.empty:
        cst = pareto_analysis(cust_agg.copy(), "sales", "cusno")
        for ci, h in enumerate(["排名","客戶編號","客戶名稱","銷售額","占比","累計占比","80/20"], 1):
...
def sheet_inventory_alert(wb, stock_agg):
    ...
    if "stock_status" in stock_agg.columns:
        ...
        title(ws, 2, 1, "庫存狀態彙總", 3)
        for ci, h in enumerate(["狀態","品項數","占比"], 1):
            hdr(ws, 3, ci, h)
    ...
    hdrs_map  = {"prdno":"商品編號","prdnm":"商品名稱","qty":"目前庫存",
                 "safeqty":"安全庫存","stock_status":"狀態","stock_value":"庫存金額",
                 "lastin":"上次進貨","lastout":"上次出貨"}
    for ci, c in enumerate(show_cols, 1):
...
def sheet_gross_margin(wb, prod_agg):
    ...
    labels = ["虧損(<0%)", "低(0-10%)", "中(10-20%)", "良(20-30%)", "高(>30%)"]
    ...
    for ci, h in enumerate(["毛利級距","品項數","銷售占比","建議"], 1):
        hdr(ws, 3, ci, h)
    total_sale = df["sales"].sum()
    # Business action for each GP tier — drives the "Suggestion" column in the report
    suggestions = {
        "虧損(<0%)":  "檢討定價或停售",         # selling below cost
        "低(0-10%)":  "考慮漲價或降低成本",     # barely profitable
        "中(10-20%)": "觀察",                    # acceptable but thin
        "良(20-30%)": "維持",                    # healthy margin
        "高(>30%)":   "主推",                    # high margin = push volume
    }
    ...
    row = 5 + len(dist) + 1
    for label, subset in [("低毛利商品（<10%，需關注）", df[df["gp_rate"] < 0.1].sort_values("sales", ascending=False).head(20)),
                          ("高毛利商品（>30%，可主推）", df[df["gp_rate"] > 0.3].sort_values("sales", ascending=False).head(20))]:
        title(ws, row, 1, label, 6)
        row += 1
        for ci, h in enumerate(["商品編號","商品名稱","銷售額","成本","毛利","毛利率"], 1):
            hdr(ws, row, ci, h)
        row += 1
...
def sheet_charts(wb, monthly, cust_agg, prod_agg):
    ...
    if not top_prod.empty:
        ws.cell(row=2, column=data_col3,   value="商品")
        ...
        chart3.type   = "bar"
        chart3.style  = 10
        chart3.title  = "TOP 10 商品銷售額"
        chart3.height = 14
        chart3.width  = 20
        chart3.y_axis.title = "商品"
