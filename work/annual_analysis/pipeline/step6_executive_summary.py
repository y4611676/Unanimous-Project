"""
Step 6: Annual Executive Summary → Excel Dashboard

One-page boss-friendly summary optimised for a full-year data window.
Four sections:
  ① 本年度整體表現   — 全年營收 + 年增率（YoY）＋各季分佈
  ② 最重要的發現    — 1–3 項自動偵測的關鍵洞察
  ③ 需要老闆決定的事 — 決策清單（非系統能自動解決的）
  ④ 下季預測        — 未來3個月銷售預測數字 + 折線圖

Reads same aggregated/ folder as step3/step5.
Outputs:  annual_executive_summary.xlsx  (in the folder's parent)
"""
...
def _import_step5():
    """Dynamically load step5 to reuse sales_forecast() without circular imports."""
...
def current_year_stats(monthly):
    """
    Return (label, this_year_sales, prev_year_sales, pct_yoy).
    Uses the latest calendar year found in the data.
    """
    ...
    by_year     = df.groupby("year", sort=True)["sales"].sum()
    if by_year.empty:
        return "N/A", 0.0, 0.0, 0.0
    this_year  = str(by_year.index[-1])
    this_total = float(by_year.iloc[-1])
    prev_total = float(by_year.iloc[-2]) if len(by_year) >= 2 else 0.0
    pct        = (this_total - prev_total) / prev_total if prev_total else 0.0
    return this_year, this_total, prev_total, pct


def quarterly_breakdown(monthly, year_label):
    """Return DataFrame [quarter, sales] for the given year, sorted Q1→Q4."""
    if monthly.empty or "sales" not in monthly.columns:
        return pd.DataFrame(columns=["quarter", "sales"])
    df = monthly.copy()
    df["sales"]   = pd.to_numeric(df["sales"], errors="coerce").fillna(0)
    df["year_str"]= df["ym"].astype(str).str[:4]
    df["quarter"] = df["ym"].astype(str).apply(_ym_to_quarter)
    year_data     = df[df["year_str"] == str(year_label)]
    if year_data.empty:
        return pd.DataFrame(columns=["quarter", "sales"])
    return (year_data.groupby("quarter", sort=True)["sales"]
...
def build_findings(monthly, cust_agg, prod_agg, stock_agg):
    ...
    # Finding 1 – Inventory risk (slow-moving / zero-stock)
    ...
    # Finding 4 – Full-year revenue trend (H1 vs H2)
    if not monthly.empty and "sales" in monthly.columns:
        ...
        if len(df) >= 6:
            mid   = len(df) // 2
            h1    = float(df.iloc[:mid]["sales"].mean())
            h2    = float(df.iloc[mid:]["sales"].mean())
            if h1 > 0:
                t    = (h2 - h1) / h1
                desc = "成長" if t > 0 else "下降"
                note = "下半年持續成長，年末衝刺有望" if t > 0 else "下半年趨緩，需檢討市場策略"
                if abs(t) >= 0.05:
                    findings.append({
                        "title":  f"上下半年銷售{'成長' if t > 0 else '落差'}",
                        "body":   (
                            f"下半年月均銷售較上半年{desc} {abs(t):.0%}，{note}。"
                        ),
                        "action": None,
                    })

    # Finding 5 – Hot sellers with thin margins
...
def build_decisions(findings):
    ...
    decisions.append({
        "category": "明年規劃",
        "question": "根據本年度數字與下季預測，明年銷售目標、重點客戶與產品策略是否需要調整？",
    })
    return decisions[:5]
...
def sheet_executive_summary(wb, monthly, cust_agg, prod_agg, stock_agg, s5):
    ...
    # ──────────────────────────────────────────────────────────────────────────
    _m(ws, 1, 1, 1, 10)
    banner = ws.cell(row=1, column=1, value="年度經營摘要儀表板")
    ...
    sub = ws.cell(row=2, column=1,
                  value=f"產生時間：{ts}　　｜　　本報告由系統自動彙整，供年度決策參考")
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # Section ① — 本年度整體表現 (rows 4–11)
    # ──────────────────────────────────────────────────────────────────────────
    _sec_header(ws, 4, "①  本年度整體表現", bg=MED_BLUE)

    year_label, this_year, prev_year, pct_yoy = current_year_stats(monthly)
    arrow     = "▲" if pct_yoy >= 0 else "▼"
    pct_color = GREEN_SOFT if pct_yoy >= 0 else RED

    # Card A: 本年度營收 (cols 2–5)
    _m(ws, 5, 5, 2, 5)
    ca_h = ws.cell(row=5, column=2, value="本年度營收")
    ...
    _m(ws, 6, 6, 2, 5)
    ca_v = ws.cell(row=6, column=2, value=this_year)
    ...
    _m(ws, 7, 7, 2, 5)
    ca_s = ws.cell(row=7, column=2, value=f"年度：{year_label}")
    ...
    # Card B: 與去年相比，YoY (cols 6–9)
    _m(ws, 5, 5, 6, 9)
    cb_h = ws.cell(row=5, column=6, value="與去年相比（YoY）")
    ...
    _m(ws, 6, 6, 6, 9)
    cb_v = ws.cell(row=6, column=6, value=f"{arrow}  {abs(pct_yoy):.1%}")
    ...
    _m(ws, 7, 7, 6, 9)
    cb_s = ws.cell(row=7, column=6, value=f"去年營收：{prev_year:,.0f}")
    ...
    # ── Quarterly breakdown bar within the year (cols 2–9, rows 9–11) ─────
    q_df = quarterly_breakdown(monthly, year_label)
    if not q_df.empty:
        _m(ws, 9, 9, 2, 9)
        qh = ws.cell(row=9, column=2, value="各季銷售分佈")
        qh.font      = Font(name="Arial", bold=True, size=9, color=DARK_BLUE)
        qh.fill      = PatternFill("solid", fgColor=LIGHT_BLUE)
        qh.alignment = Alignment(horizontal="left", vertical="center", indent=1)
        ws.row_dimensions[9].height = 16

        q_total = float(q_df["sales"].sum()) or 1.0
        qbar_cols = [(2, 3), (4, 5), (6, 7), (8, 9)]
        for qi, (_, qrow) in enumerate(q_df.iterrows()):
            if qi >= 4:
                break
            cs, ce = qbar_cols[qi]
            _m(ws, 10, 10, cs, ce)
            qlab = ws.cell(row=10, column=cs, value=str(qrow["quarter"]))
            qlab.font      = Font(name="Arial", bold=True, size=9, color=DARK_BLUE)
            qlab.fill      = PatternFill("solid", fgColor=GRAY)
            qlab.alignment = Alignment(horizontal="center", vertical="center")
            _m(ws, 11, 11, cs, ce)
            qval = ws.cell(row=11, column=cs, value=round(float(qrow["sales"]), 0))
            qval.font          = Font(name="Arial", bold=True, size=10, color=MED_BLUE)
            qval.fill          = PatternFill("solid", fgColor="EEF4FB")
            qval.alignment     = Alignment(horizontal="center", vertical="center")
            qval.number_format = "#,##0"
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # Section ② — 最重要的發現 (dynamic rows, starting at 13)
    # ──────────────────────────────────────────────────────────────────────────
    _sec_header(ws, 13, "②  最重要的發現", bg=TEAL)
    row = 14

    findings = build_findings(monthly, cust_agg, prod_agg, stock_agg)
    if not findings:
        _m(ws, row, row, 2, 9)
        _c(ws, row, 2, "資料完整後將自動產生重要發現。", fg="888888", size=10)
    ...
    if forecast_df.empty:
        ...
        # Chart data written to off-screen columns (col 12+)
        DATA_COL  = 12
    ...
    content_end = max(row - 1, 13)
    _thin_border(ws, 4, content_end, 1, 10)

    # ── Print / PDF layout — lock everything so the sheet never jumps ─────────
    # fitToWidth=1 squeezes all columns onto one page width (no right overflow).
    # fitToHeight=0 means "auto": let it grow as many pages tall as it needs.
    ...
    # Locking the print area prevents Excel from auto-recalculating page breaks
    # every time the file is opened with different window/zoom settings.
...
def main():
    ...
    out_path = src.parent / "annual_executive_summary.xlsx"
    print(f"\nSource : {src}")
