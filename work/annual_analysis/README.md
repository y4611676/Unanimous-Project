# 年度分析工具 (Annual Analysis)

每年結束後，將整年度 ERP 原始 CSV 餵入管線，自動產出三份 Excel 報表供年度決策與覆盤使用。
...
└── pipeline/
    ...
    └── step6_executive_summary.py  年度經營摘要（給老闆）
```
...
# Step 6：年度經營摘要（一頁給老闆看）
python step6_executive_summary.py
# 輸出：annual_executive_summary.xlsx
```
...
| `annual_executive_summary.xlsx` | 年度經營摘要（單頁） | 總經理、老闆 |
...
年度版的 Step 6 與季度版有以下差異：

- **①  本年度整體表現**：顯示全年總營收 + **YoY 年增率**（而非季增率）  
  並附上 Q1–Q4 各季分佈，讓老闆一眼看出哪一季最強。
- **②  最重要的發現**：偵測上下半年趨勢差距（H1 vs H2），而非短期週期。
- **③  需要老闆決定的事**：以「明年規劃」為收尾決策項，幫助年度策略討論。
- **④  下季預測**：預測下一年 Q1 開頭的 3 個月走勢。

---

## 與 quarterly_analysis 的差異
...
- Step 5 的季節性分解需要至少 **13 個月**月度資料；銷售預測需要至少 **6 個月**。
- 年度分析建議使用完整 12 個月的資料，確保 YoY 比較有意義。
- 建議每年建立獨立的工作資料夾，避免覆蓋前一年的輸出。
- `step5_advanced.py` 含有流失風險評分（邏輯迴歸），需要 `scikit-learn`，已包含在 `requirements.txt`。
